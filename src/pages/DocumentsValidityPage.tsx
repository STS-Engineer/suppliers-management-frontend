...
const STATUS_CFG: Record<ValidityStatus, { cls: string; dot: string; label: string }> = {
  expired:  { cls: "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300",                dot: "bg-red-500",     label: "Expired"  },
  expiring: { cls: "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300",        dot: "bg-amber-500",   label: "Expiring" },
  valid:    { cls: "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300", dot: "bg-emerald-500", label: "Valid"    },
  missing:  { cls: "bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400",               dot: "bg-gray-400",    label: "No date"  },
};

function StatusBadge({ status }: { status: ValidityStatus }) {
  const c = STATUS_CFG[status];
  return (
    <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold ${c.cls}`}>
...
function DocCell({ url, name, evidenceFile }: { url: string | null; name: string | null; evidenceFile: string | null }) {
  const [open, setOpen] = useState(false);

  if (!url && !evidenceFile) return <span className="text-[11px] text-gray-400">No document</span>;
      ...
      {open && url && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/70 backdrop-blur-sm"
          ...
          <div className="relative max-h-[90vh] max-w-[90vw]" onClick={(e) => e.stopPropagation()}>
            ...
            <img src={url} alt={label} className="max-h-[88vh] max-w-[88vw] rounded-xl shadow-2xl object-contain" />
            <a href={url} target="_blank" rel="noopener noreferrer"
...
function ResetLog({ log, onClose }: { log: ResetLogEntry[]; onClose: () => void }) {
  return (
    <div className="mb-5 rounded-2xl border border-emerald-200 bg-emerald-50 shadow-sm dark:border-emerald-500/20 dark:bg-emerald-900/10">
      <div className="flex items-center justify-between border-b border-emerald-200 px-5 py-3 dark:border-emerald-500/20">
        ...
        <button onClick={onClose} className="text-emerald-500 hover:text-emerald-700"><X size={14} /></button>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-emerald-200 dark:border-emerald-500/20">
              {["Supplier", "Site", "Criterion", "Previous value", "Expired on", "Reason"].map((h) => (
                <th key={h} className="px-4 py-2 text-left text-[10px] font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-500">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {log.map((entry, i) => (
              <tr key={i} className="border-t border-emerald-100 dark:border-emerald-500/10">
                <td className="px-4 py-2 text-[12px] font-semibold text-gray-800 dark:text-gray-200">{entry.supplierName}</td>
                <td className="px-4 py-2 text-[12px] text-gray-500 dark:text-gray-400">{entry.siteName}</td>
                <td className="px-4 py-2 text-[12px] font-medium text-gray-700 dark:text-gray-300">{entry.criterionLabel}</td>
                <td className="px-4 py-2">
                  {entry.previousValue
                    ? <span className="rounded-md bg-red-50 px-2 py-0.5 text-[11px] font-medium text-red-700 line-through dark:bg-red-900/20 dark:text-red-400">{entry.previousValue}</span>
                    : <span className="text-[11px] text-gray-400">—</span>}
                </td>
                <td className="px-4 py-2 text-[12px] font-semibold text-red-600 dark:text-red-400">{fmtDate(entry.endDate)}</td>
                <td className="px-4 py-2 text-[11px] italic text-gray-500 dark:text-gray-400">{entry.reason}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function ResultsPanel({
  ...
  const filtered = useMemo(() => {
    let list = allEntries;
    if (statusFilter === "expired")  list = list.filter((e) => e.status === "expired");
    if (statusFilter === "expiring") list = list.filter((e) => e.status === "expiring");
    if (search.trim()) {
  ...
  const groups = useMemo(() => {
    if (groupBy === "supplier") {
      const map = new Map<string, { label: string; sub: string; entries: CriterionEntry[] }>();
      for (const e of filtered) {
        const key = `${e.relId}`;
        if (!map.has(key)) map.set(key, { label: e.supplierName, sub: e.siteName, entries: [] });
        map.get(key)!.entries.push(e);
      }
      return Array.from(map.entries());
    } else {
      const map = new Map<string, { label: string; sub: string; entries: CriterionEntry[] }>();
      for (const c of CLASS_CRITERIA) map.set(c.key, { label: c.label, sub: "", entries: [] });
      for (const e of filtered) {
    ...
    <div className="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-white/10 dark:bg-gray-900">
      ...
      <div className="flex flex-wrap items-center gap-3 border-b border-gray-100 px-5 py-3.5 dark:border-white/[0.06]">
        ...
        <div className="flex items-center gap-1.5">
          {(["expired", "expiring", "all"] as const).map((s) => {
            ...
            const color =
              s === "expired"  ? active ? "bg-red-600 text-white border-red-600"     : "border-red-200 text-red-600 hover:bg-red-50"      :
              s === "expiring" ? active ? "bg-amber-500 text-white border-amber-500" : "border-amber-200 text-amber-600 hover:bg-amber-50" :
                                 active ? "bg-gray-600 text-white border-gray-600"   : "border-gray-200 text-gray-600 hover:bg-gray-50";
            return (
              <button key={s} onClick={() => setStatusFilter(s)}
                ...
                <span className={`rounded-full px-1.5 text-[9px] font-bold ${active ? "bg-white/25" : "bg-gray-100 dark:bg-white/10"}`}>{count}</span>
              </button>
            );
          })}
        </div>
        ...
        <div className="flex items-center overflow-hidden rounded-xl border border-gray-200 dark:border-white/10">
          <button onClick={() => setGroupBy("supplier")}
          ...
          <button onClick={() => setGroupBy("criterion")}
        ...
        <span className="text-[11px] text-gray-400">{filtered.length} entries</span>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button onClick={onRefresh} disabled={refreshing}
            ...
            {refreshing ? <Loader2 size={12} className="animate-spin" /> : <ScanLine size={12} />}
            Refresh
          </button>
          ...
          {expiredEntries.length > 0 && (
            confirmReset ? (
              <>
                ...
                <button onClick={() => { onReset(expiredEntries); setConfirmReset(false); }}
                  disabled={resetting}
                ...
                <button onClick={() => setConfirmReset(false)} className="text-xs text-gray-500 hover:text-gray-700">Cancel</button>
              </>
            ) : (
      ...
      {groups.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">No {statusFilter === "all" ? "" : statusFilter} criteria found.</p>
        </div>
      ) : (
        <div className="overflow-x-auto">
          {groups.map(([groupKey, group]) => {
            const isCollapsed    = collapsed.has(groupKey);
            const expiredInGroup  = group.entries.filter((e) => e.status === "expired").length;
            const expiringInGroup = group.entries.filter((e) => e.status === "expiring").length;
            ...
            return (
              <div key={groupKey}>
                ...
                >
                  ...
                  <span className="text-sm font-bold text-gray-800 dark:text-gray-100">{group.label}</span>
                  {group.sub && <span className="text-[11px] text-gray-500 dark:text-gray-400">· {group.sub}</span>}
                  ...
                  <span className="ml-auto text-[11px] text-gray-400">{group.entries.length} entries</span>
                </button>
                  ...
                  <table className="w-full min-w-[900px] text-sm">
                    <thead>
                      <tr className="border-b border-gray-100 dark:border-white/[0.04]">
                        {(groupBy === "supplier"
                          ? ["Criterion", "Value", "Valid From", "Valid Until", "Days Left", "Status", "Document", ""]
                          : ["Supplier", "Avocarbon Site", "Value", "Valid From", "Valid Until", "Days Left", "Status", "Document", ""]
                        ).map((h) => (
                          <th key={h} className="px-4 py-2 text-left text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {group.entries.map((entry, idx) => {
                        const rowBg =
                          entry.status === "expired"  ? "bg-red-50/60 dark:bg-red-900/10"     :
                          entry.status === "expiring" ? "bg-amber-50/50 dark:bg-amber-900/10" : "";
                        return (
                          <tr key={idx} className={`border-t border-gray-100 dark:border-white/[0.04] ${rowBg}`}>
                            {groupBy === "supplier" ? (
                              <td className="px-4 py-2.5 text-[12px] font-semibold text-gray-700 dark:text-gray-300">{entry.criterionLabel}</td>
                            ) : (
                              <>
                                <td className="px-4 py-2.5 text-[12px] font-semibold text-gray-800 dark:text-gray-100">{entry.supplierName}</td>
                                <td className="px-4 py-2.5 text-[12px] text-gray-500 dark:text-gray-400">{entry.siteName}</td>
                              </>
                            )}
                            <td className="px-4 py-2.5">
                              {entry.value
                                ? <span className="inline-block max-w-[160px] truncate rounded-md bg-blue-50 px-2 py-0.5 text-[11px] font-medium text-blue-700 dark:bg-blue-900/20 dark:text-blue-300">{entry.value}</span>
                                : <span className="text-[11px] text-gray-400">—</span>}
                            </td>
                            <td className="px-4 py-2.5 text-[12px] text-gray-500">{fmtDate(entry.startDate)}</td>
                            <td className="px-4 py-2.5 text-[12px] font-medium text-gray-700 dark:text-gray-300">{fmtDate(entry.endDate)}</td>
                            <td className="px-4 py-2.5 text-[12px] tabular-nums">
                              {entry.days === null
                                ? <span className="text-gray-400">—</span>
                                : entry.days < 0
                                  ? <span className="font-bold text-red-600 dark:text-red-400">{Math.abs(entry.days)}d ago</span>
                                  : <span className={entry.days <= 30 ? "font-bold text-red-600" : entry.days <= 90 ? "font-bold text-amber-600" : "text-emerald-600"}>{entry.days}d</span>}
                            </td>
                            <td className="px-4 py-2.5"><StatusBadge status={entry.status} /></td>
                            <td className="px-4 py-2.5">
                              <DocCell url={entry.documentUrl} name={entry.documentName} evidenceFile={entry.evidenceFile} />
                            </td>
                            <td className="px-4 py-2.5">
                              <Link to={`/supplier-relations/${entry.relId}/evaluation`}
...
export default function DocumentsValidityPage() {
  ...
  const relMeta = useMemo<Record<number, { supplierName: string; siteName: string }>>(() => {
    ...
    for (const item of relations) {
      ...
      map[relId] = {
        ...
        siteName:     sites[item.relation.id_site!] ?? `Site #${item.relation.id_site}`,
      };
    }
    return map;
  }, [relations, sites]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([panelRes, sitesRes, bulkRes]) => {
        if (cancelled) return;
        const siteList: any[] = Array.isArray(sitesRes?.data) ? sitesRes.data : Array.isArray(sitesRes) ? sitesRes : [];
        const siteMap: Record<number, string> = {};
        for (const s of siteList) if (s.id_site) siteMap[s.id_site] = s.site_name ?? `Site #${s.id_site}`;
        setSites(siteMap);
  ...
  async function handleResetAllExpired(expiredEntries: CriterionEntry[]) {
    ...
    log.sort((a, b) => a.supplierName.localeCompare(b.supplierName) || a.criterionLabel.localeCompare(b.criterionLabel));
    setResetLog(log);
    setResetting(false);
  ...
  const allEntries = useMemo<CriterionEntry[]>(() => {
    ...
    entries.sort((a, b) => {
      const order: Record<ValidityStatus, number> = { expired: 0, expiring: 1, missing: 2, valid: 3 };
  ...
  const totalExpiring = allEntries.filter((e) => e.status === "expiring").length;
  ...
  return (
    <div className="mx-auto flex max-w-[1700px] flex-col gap-5 px-2">
      <PageIntro
      ...
      <div className="flex flex-col gap-5 mx-auto w-full max-w-[1600px]">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {kpiCards.map((c) => {
            ...
            const active    = c.filter !== null && statusFilter === c.filter && scanDone;
            return (
              <div
                key={c.label}
                onClick={() => { if (clickable) setStatusFilter(c.filter!); }}
                className={[
              ...
              >
                <div className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl ${c.bg} ${c.cls}`}>{c.icon}</div>
                <div>
        ...
        {error && <InlineAlert title="Failed to load relations" message={error} tone="danger" />}

        {/* Reset log */}
        {resetLog && <ResetLog log={resetLog} onClose={() => setResetLog(null)} />}

        {/* Results — always shown; spinner while initial load, table once data arrives */}
        {loading ? (
          <div className="flex items-center justify-center gap-2 py-16 text-sm text-gray-500">
            <Loader2 size={16} className="animate-spin text-blue-500" /> Loading criteria data…
          </div>
        ) : (
          <ResultsPanel
