...
const matchesMultiValue = (
  value: string | null | undefined,
  needle: string,
) => {
  const key = normalizeText(needle);
  return splitMultiValue(value).some((part) => {
    ...
    <div className="flex flex-wrap gap-1.5">
      {parts.map((part) => (
        <span
          key={part}
          title={part}
          className={`inline-flex items-center rounded-md px-2 py-0.5 leading-tight ${
            variant === "strong"
              ? `text-[11px] font-semibold ${tagColor(part)}`
              : `text-[10.5px] font-medium ${tagColor(part)}`
          }`}
          style={variant === "subtle" ? { opacity: 0.9 } : undefined}
        >
          {formatTaxonomyLabel(part)}
        </span>
      ))}
    </div>
  );
  ...
  return (
    <div className="mx-4 mb-3 overflow-hidden rounded-2xl border border-slate-100/80 bg-white shadow-sm dark:border-white/[0.06] dark:bg-[#0f1e30]">
      <div className="flex items-center gap-2.5 border-b border-slate-50/80 px-4 py-3 dark:border-white/[0.04]">
        {icon && (
          <span
            className={`flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-lg ${accentCls}`}
          >
            {icon}
          </span>
        )}
...
function RelationDetailModal({
  ...
  const gradeBg: Record<string, string> = {
    green:
      "bg-emerald-50 ring-1 ring-emerald-200 dark:bg-emerald-500/10 dark:ring-emerald-500/20",
    amber:
    ...
    slate:
      "bg-slate-100 ring-1 ring-slate-200 dark:bg-white/[0.06] dark:ring-white/[0.08]",
    purple:
      "bg-violet-50 ring-1 ring-violet-200 dark:bg-violet-500/10 dark:ring-violet-500/20",
  };
              ...
              <div className="space-y-2 px-4 py-2.5">
                {record.unit.family && (
                  <div className="space-y-1">
                    ...
                    <TaxonomyChips value={record.unit.family} variant="strong" />
                  </div>
                )}
                {record.unit.sub_family && (
                  <div className="space-y-1">
                    ...
                    <TaxonomyChips value={record.unit.sub_family} />
                  </div>
                )}
                {record.unit.product_line && (
                  <div className="space-y-1">
                    ...
                    <TaxonomyChips value={record.unit.product_line} />
                  </div>
                )}
              </div>
            )}
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                {(
                  [
                    ...
                    {
                      label: "Global scope",
                      value: record.relation.global_status,
                    },
              ...
              <div className="flex items-center justify-between gap-3 px-4 py-2.5 transition-colors hover:bg-slate-50/60 dark:hover:bg-white/[0.02]">
                ...
                <span
                  className={`inline-flex items-center rounded-lg px-2.5 py-0.5 text-[11px] font-bold ring-1 ${
                    GRADE_TILE[
                      getGradeTone(record.relation.operational_grade)
...
export default function ActiveSuppliersPage() {
  ...
  const [committeeProgressRow, setCommitteeProgressRow] =
  ...
  const filterOptions = useMemo(() => {
    ...
    const uniqFlattened = (arr: (string | null | undefined)[]) =>
      [...new Set(arr.flatMap((value) => splitMultiValue(value)))].sort(
        (a, b) => a.localeCompare(b),
      );
    return {
      statuses: uniq(relationRows.map((r) => r.relation.supplier_status)),
      families: uniqFlattened(relationRows.map((r) => r.unit.family)),
      subFamilies: uniqFlattened(relationRows.map((r) => r.unit.sub_family)),
      productLines: uniqFlattened(relationRows.map((r) => r.unit.product_line)),
  ...
  const filteredRows = useMemo(() => {
    ...
    if (filterAlias) {
      ...
      rows = rows.filter((row) =>
        normalizeText(row.relation.alias_1 ?? "").includes(kw),
      );
    }
    if (filterGroupName) {
    ...
    if (filterUnitName) {
      ...
      rows = rows.filter((row) =>
        normalizeText(row.unit.supplier_name).includes(kw),
      );
    }
    if (filterPlant) {
      const kw = normalizeText(filterPlant);
      rows = rows.filter((row) =>
        normalizeText(row.site.site_name).includes(kw),
      );
    }
    if (filterFamily) {
      rows = rows.filter((row) =>
        matchesMultiValue(row.unit.family, filterFamily),
      );
    }
    if (filterSubFamily) {
      rows = rows.filter((row) =>
        matchesMultiValue(row.unit.sub_family, filterSubFamily),
      );
    }
    ...
    <div className="flex min-h-[calc(100vh-160px)] flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-1 rounded-xl border border-slate-200 bg-white p-1 shadow-sm dark:border-white/[0.08] dark:bg-[#111e30]">
          {panelTabs
            .filter((tab) => tab.key !== "panel_add_exec_committee" || isAdmin)
            .map((tab) => {
              ...
              <tbody>
                {Array.from({ length: 8 }).map((_, i) => (
                  ...
                  >
                    ...
                    {/* Status */}
                    <td className="px-5 py-3.5">
                      <div className="h-6 w-28 animate-pulse rounded-full bg-slate-100 dark:bg-white/[0.06]" />
                    </td>
                    {/* Family */}
                    <td className="px-5 py-3.5">
                    ...
                    {/* Product line */}
                    <td className="px-5 py-3.5">
                      <div className="h-5 w-24 animate-pulse rounded-md bg-slate-100 dark:bg-white/[0.06]" />
                    </td>
                    {/* Grade */}
                    <td className="px-5 py-3.5">
                  ...
                  return (
                    ...
                    >
                        ...
                        {row.relation.global_status ? (
                          <span
                            className={`inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-semibold ${
                              normalizeText(row.relation.global_status) ===
                              "global"
                      ...
                      <td className="px-5 py-3.5">
                        <Badge
                          text={
                            row.relation.supplier_status || "Status pending"
                          }
                      ...
                      <td className="px-5 py-3.5 align-top">
                        <TaxonomyChips value={row.unit.family} variant="strong" />
                      </td>

                      {/* Sub-family */}
                      <td className="px-5 py-3.5 align-top">
                        <TaxonomyChips value={row.unit.sub_family} />
                      </td>

                      {/* Product line */}
                      <td className="px-5 py-3.5 align-top">
                        <TaxonomyChips value={row.unit.product_line} />
                      </td>
                      ...
                      <td className="px-5 py-3.5">
                        <StrategicChips
                          value={row.relation.strategic_mention}
                        />
                      </td>
                        ...
                        <div className="flex flex-wrap items-center justify-end gap-1.5">
                          ...
                          {isVpConversion && (
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-sm transition-colors hover:border-amber-200 hover:bg-amber-50 hover:text-amber-700"
                            >
                              Override Status
                            </button>
                          )}
                          ...
                          {isVpConversion &&
                            ...
                            (row.committee_review_status === "in_progress" ||
                            row.committee_review_status === "pending_final" ? (
                              <button
                                type="button"
                                onClick={(e) => {
                                  e.stopPropagation();
          ...
          >
            ...
            <div className="flex items-center justify-end gap-3 border-t border-slate-100 px-6 py-4">
              ...
              <button
                ...
                className="rounded-xl border border-amber-200 bg-amber-500 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-amber-600 disabled:opacity-50 disabled:cursor-not-allowed"
              >
          ...
          >
            ...
            >
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs text-white/70">
                  {confirmToggleRow.group.nom} —{" "}
                  {confirmToggleRow.site.site_name}
                </p>
              </div>
            </div>
            ...
            <div className="px-6 py-5">
              <p className="text-sm text-slate-700">
                {(confirmToggleRow.relation.is_active ?? true) ? (
                  <>
                    This relation will be marked <strong>inactive</strong> and
                    will no longer appear in the supplier panel, public
                    directory, or opportunity tracking for this site.
                  </>
                ) : (
                  <>
                    This relation will be marked <strong>active</strong> again
                    and will resume appearing in the supplier panel and
            ...
            <div className="flex items-center justify-end gap-3 border-t border-slate-100 px-6 py-4">
              ...
              <button
                type="button"
                disabled={
                  togglingUnit === confirmToggleRow.relation.id_relation
                }
                onClick={async () => {
