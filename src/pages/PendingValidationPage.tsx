...
function SupplierCard({
  ...
  const handleApprove = async () => {
    ...
    setError(null);
    try {
      await supplierAPI.approveSupplier(item.group_id, comment || undefined);
      onDone();
    } catch (e: any) {
      setError(e.message ?? "Failed to approve supplier.");
      setLoading(false);
    }
  };
  ...
  const handleReject = async () => {
    if (!comment.trim()) {
      setError("A reason is required to reject a supplier.");
      return;
    }
    setLoading(true);
    setError(null);
    try {
      await supplierAPI.rejectSupplier(item.group_id, comment);
      onDone();
    } catch (e: any) {
      setError(e.message ?? "Failed to reject supplier.");
      setLoading(false);
    }
  };
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
              {item.group_code} · {item.unit_code}
            </p>
          </div>
        </div>
      ...
      {/* Details row */}
      <div className="grid grid-cols-2 gap-3 border-t border-slate-100 px-5 py-3 sm:grid-cols-4 dark:border-slate-700">
        <Detail icon={<MapPin className="h-3.5 w-3.5" />} label="Country" value={item.unit_country} />
        <Detail icon={<Building2 className="h-3.5 w-3.5" />} label="Site" value={item.site_name} />
        <Detail
          icon={<Building2 className="h-3.5 w-3.5" />}
          label="Scope"
          value={scopeLabel[item.supplier_scope ?? ""] ?? item.supplier_scope}
        />
          ...
          {action === "idle" && (
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2"
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-red-300 bg-white px-4 py-2 text-sm font-semibold text-red-600 shadow-sm hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:border-red-700 dark:bg-slate-800 dark:text-red-400"
              >
            ...
            <div className="space-y-3">
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-700 dark:text-slate-300">
                  {action === "approving" ? "Comment (optional)" : "Reason for rejection *"}
                </label>
                <textarea
                  value={comment}
                  ...
                  placeholder={
                    action === "approving"
                      ...
                      : "Explain why this supplier is being rejected…"
                  }
                  className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100"
                />
              </div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={() => { setAction("idle"); setComment(""); setError(null); }}
                  disabled={loading}
    ...
    <div className="mx-auto max-w-4xl px-2 py-6">
      ...
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          New suppliers awaiting your approval before they are added to the panel.
        </p>
      </div>
      ...
      {!loading && !error && items.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 py-16 text-center dark:border-slate-600">
          ...
          <p className="mt-1 text-xs text-slate-500">No suppliers pending validation.</p>
        </div>
      )}
